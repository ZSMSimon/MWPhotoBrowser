    ...
    [delAlert addAction:[UIAlertAction actionWithTitle:@"删除" style:UIAlertActionStyleDestructive handler:^(UIAlertAction * _Nonnull action) {
        if (self->_photoCount > self->_currentPageIndex) {
            if ([self->_delegate respondsToSelector:@selector(removeFromPhotosAtIndex:)]) {
                [self->_delegate removeFromPhotosAtIndex:self->_currentPageIndex];
                [weakSelf reloadData];
                
                if (self.removeBlock) {
                    self.removeBlock(self->_currentPageIndex);
                }
                if (self->_photoCount == 0) {
                    [self.navigationController popViewControllerAnimated:YES];
                }
            }
        }
    }]];
    [delAlert addAction:[UIAlertAction actionWithTitle:@"取消" style:UIAlertActionStyleCancel handler:nil]];
